...
- **`02-I02` 2 つのインフレ率が既定値も用途も別になっている**
  ...
  - **数値インパクト例**: ユーザーが `finance.inflationRate` だけを 3% に上書きしても、退職期支出の複利は別変数 `retirement.inflationRate` の 1.5% のまま。30 年退職シミュレーションでは `1.03^30 / 1.015^30 ≈ 2.4273 / 1.5631 ≈ 1.553` 倍の乖離になり、退職期終盤支出が実質 55% 過少推計されうる。
