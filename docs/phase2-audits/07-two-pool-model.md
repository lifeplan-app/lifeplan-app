...
- **信頼度 ❌ 要対応 の根拠**: `07-C01` は **FIRE/出口戦略シミュレーション（本アプリの中核機能）の全呼び出し経路**（§対象範囲 8 箇所）で再現する構造バグ。「30 年の退職シミュで投資プールが 25 年目に枯渇 → 残り 5 年は架空清算で資産がプラスに見える」というケースは**退職シミュの主要検証シナリオそのもの**であり、極端ケースではない。ユーザーが「資産が最後まで持つか」を判定する UI 経路で誤答が出るため、他 Phase 2 監査（Task 3/5/6/7）の Critical ≥ 1 → ❌ 方針と整合させて ❌ 要対応 とする。`_liquidationEvents` の額面記録は監査トレーサビリティを維持するが UI には露出していない。
- **申し送り**:
