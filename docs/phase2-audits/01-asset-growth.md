...
- **標準**: 運用益は非課税。受取時に退職所得控除または公的年金等控除を適用。
  ...
  - 会社員（企業年金なし）の拠出限度額は **月額2.3万円（年額27.6万円）**。2024年12月以降、段階的に改正（2024/12〜月2.3万円、2026年予定の改正あり）→ 最新動向は要確認。
- **本コード**: `ideco.monthlyLimit = 2.3` (`index.html:5338`)、`effectiveReturn` で `ideco` を非課税扱い。
- **判定**: ✅ 運用益非課税は一致。拠出限度額は「企業年金なし」前提の固定値で、企業型DC併用や第1号/第3号被保険者の別は考慮されていない（§6）。
...
- **`01-I01` 課税口座の「毎年課税」近似が長期で実態と乖離**
  ...
  - 例: 年利 5% / 30年 / 一括 1,000万円 で比較すると、実態は売却時一括課税で約 `1000 × 1.05^30 × (1 − 0.20315 × 利益分/時価)` ≈ 3,555万円。本コードの毎年課税近似は `1000 × (1 + 0.05×0.79685)^30` ≈ 2,700万円程度となり **約25%過少評価**。
  - 配当受取モードでは配当部分だけ毎年課税するのが正しいため、値上がり分と配当を分離した税処理にする余地あり。
  - 影響：出口戦略・FIRE 達成年の判定が保守的に出る。現状は「安全側」なので致命的ではないが、使いどころとしては重要。

- **`01-I02` `noNewContrib`（旧NISA）のロジックが `calcAssetGrowth` に反映されていない**
  - `ASSET_TYPES.nisa_old_*` に `noNewContrib: true` があるが、`calcAssetGrowth` では `a.monthly` をそのまま使う。ユーザーが `monthly = 0` に設定しなかった場合、2024年以降も新規積立が続いているかのように計算されてしまう。
  - 現状 `isOldNisa` は税率移管にしか使われていない（`index.html:8712, 8730`）。積立停止判定には使われていない。
  - 対応案：`isActive` の判定時に `noNewContrib && yr >= 2024` なら `annualContrib = 0` に強制する、または UI で入力を禁止する。
...
- **`01-M04` `iDeCo` 拠出限度額が会社員(企業年金なし)固定**
...
- **信頼度**: ⚠️ 中
- **一言サマリー**: 複利公式・税率値（20.315%）・新NISA の枠設定は標準と一致しており基本構造は妥当。ただし「特定口座＝毎年課税近似」(`01-I01`) により長期の課税口座残高が体系的に過少評価され、旧NISA の新規積立停止 (`01-I02`) も UI 依存で計算側には強制がない。出口戦略・FIRE 判定など上位領域の結果は概ね「保守側」だが、`01-I01` の影響は用途によっては無視できない大きさのため、上位監査（統合シミュレーション・出口戦略）へ申し送る。
