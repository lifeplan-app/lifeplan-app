...
- **6 グループ・18 Important 解決**:
  - G12（2件）: 07-I02, 07-I03 — `cef2643`
  - G6（1件）: 02-I02 — `bee694a`
  - G8（3件）: 01-I01, 01-I02, 01-I03 — `60c230d`
  - G9（4件）: 06-I01, 06-I02（近似）, 06-I03, 06-I04 — `33f50dd`
  - G5（8件）: 03-I01, 03-I02, 03-I03, 03-I05, 03-I06, 03-I07, 03-I09, 03-I10 — `0203be4`
- **テスト**: 155 / 155 グリーン維持
- **コミット構成**: setup 1 + fix 6 + SHA record 6 + 最終 docs 1 = 計 14 コミット（計画値通り）

### 実測インパクト（5 シナリオ合計）

- **G12**: 定性的改善（目的別現金の清算除外・年次再計算の利回り精度向上）、snapshot 差分は微小
- **G6**: 退職期インフレ率のマスター化、snapshot 差分は `retirement.inflationRate` 未設定シナリオで限定的
- **G8**: 既存サンプルが旧 NISA 未保有・振替サイクルなしで snapshot 差分ゼロ、コードパスは実装済み（防御層）
- **G9**: パートナー昇給反映・国民年金・加給年金の加算でリタイア期の世帯支出・手取りが変化。特にパートナー労働設定ありのシナリオで `retirePool` に顕著な差分
- **G5**: snapshot 合計 5,560 行変化（2,780 + / 2,780 -）。介護一時費用（C/D/E の 2040/2035/2048 で +47.2 万円）、幼稚園補正（B の 2026 で +12.46 万円）、JASSO type2 延長（A/D/E で 1 年延長）が主なドライバー。保育料・育休・双子は条件非該当で snapshot 効果なしだがコードパス稼働

### Phase 4b で対応しなかった Important（Phase 4c 対象、7 件）

| ID | 内容 | ブロッカー |
|---|---|---|
| 02-I03 | `cashFlowEvents.income_change` 適用時の昇給モデル完全停止 | G7 要実装、cashFlowEvents 統合シム側の振る舞い再設計 |
| 05-I01 | 住宅ローン子育て特例 | UI 拡張必要 |
| 05-I02 | 頭金フィールド | UI 拡張必要 |
| 05-I03 | シナリオ連動 | UI 拡張必要 |
| 05-I04 | 借換諸費用 | ローン計算拡張 |
| 05-I05 | NaN 伝播 | UI バリデーション |
| 05-I06 | 同年複数イベント順序 | 計算順序リファクタ |
| 06-I02（本実装） | `calcTakeHome` への配偶者控除の正式組込 | Phase 4b では近似対応 |

### 補足

- iDeCo 受給方法 UI（一時金 / 年金 / 併用、受給年齢 60-75 歳選択）は Important ではないが、Phase 4a `07-I04`/`08-I02` の発展として Phase 4c 以降で検討可能。
- Phase 2 監査レポート（18 件に Resolved 注記）・サニティウォークスルー（Phase 4b 再評価セクション追記）・本ファイルを最終 commit `docs(phase4b): mark Importants resolved and update walkthrough` で更新。
